...
if nargin == 1
    T = max(x) - min(x);
    [xxx,xxx,dt_samp] = find( sort(diff(sort(x))),1,'first');
    ...
    T = max(tin) - min(tin);
    x_ab = x( logical((x >= min(tin)) .*(x <= max(tin))) ) ;
    [xxx,xxx,dt_samp] = find( sort(diff(sort(x_ab))),1,'first');
...
for i = M:-1:1
    ...
    [xxx,n] = min(C_local,[],1); %find optw at t=1...L
    optws(i,:) = W(n);
end
