...
for i=1:k,
    ...
    % edit by RJE: sometimes it seems we don't get the right counts for Fij
    % and Hj. We'll just have to return "-1" in this case as a warning
    
    if numel(Fij) ~= numel(Hj)
        % problem
        %warning('numel(Fij) ~= numel(Hj). An "Inf" may be present in the data')
        break_it = 1;
        break
    else
        break_it = 0;
...
%% outputs (added by RJE)

if break_it == 0
    % stats are valid
    AnDarksamtest.result = 'The AD test was successful.';
    AnDarksamtest.noties_stat = ADKn;
    AnDarksamtest.noties_pval = Pn;
    AnDarksamtest.ties_stat = ADK;
    AnDarksamtest.ties_pval = P;
    AnDarksamtest.alpha = alpha;
else
    % stats are NOT valid; need to code this in a way that we can detect it
    AnDarksamtest.result = 'Warning: the AD test was not successful.';
    AnDarksamtest.noties_stat = -1;
    AnDarksamtest.noties_pval = -1;
    AnDarksamtest.ties_stat = -1;
    AnDarksamtest.ties_pval = -1;
    AnDarksamtest.alpha = alpha;    
end
