...
part_corrs = zeros(nperm,nreg); % individual runs as appropriate
...
elseif opt == 3
   ...
   datap = data(xperm); % a reordering of the data
       ...
       pc = partialcorr(X,Y);
       part_corrs(p,m) = pc(1,2); % just the correlation value
...
elseif opt == 2 
    ...
    regr2 = regr(x,:);
       ...
       for m = 1:nreg
           ...
           pc = partialcorr(X,Y);
           part_corrs(n,m) = pc(1,2); % just the correlation value
