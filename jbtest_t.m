function [H,P,JBSTAT,CV] = jbtest_t(x,alpha,mctol)

% A modified version of JBTEST Jarque-Bera hypothesis test of composite normality.
%
% jbtest_t evaluates "normality" with respect to a Student's T
% distribution.
...
usedist = input('\n Test against [n]ormal dist. or Student [t] dist.? ','s');

if usedist == 'n'
    kurtt = 0;
elseif usedist == 't'
    subs = input('\n Enter the number of subjects in the t-test: ');
    kurtt = 6 / (subs - 1 - 4);
end
...
ssize = length(x);                 % sample size
zscores = (x-mean(x))/std(x,1);
skews = sum(zscores.^3)/ssize      % sample skewness
kurts = sum(zscores.^4)/ssize - 3  % sample kurtosis


JBSTAT = ssize*(skews.*skews/6 + kurts.*kurts/24)

JBSTAT2 = (ssize/6) * (skews^2 + (1/4)*(kurts - kurtt)^2)
...
while true
    ...
    for i = 1:k:length(JBstatMC)
        ...
        skews = sum(devs.*sqdevs,2)./(s2.^1.5 * n);
        kurts = sum(sqdevs.*sqdevs,2)./(s2.^2 * n) - 3;
        JBstatMC(i:i+k-1) = n*(skews.*skews/6 + kurts.*kurts/24);
    end
